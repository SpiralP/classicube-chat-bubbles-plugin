...
fn panic_hook(info: &PanicHookInfo<'_>) {
    ...
    let (popup_message, stderr_message, verbose_message) = {
        ...
        (
            format!(
                "chat-bubbles crashed: '{panic_message}', {panic_location}\nMore details were \
                 written to 'chat-bubbles-crashes.log'\nPlease report this file to a developer!"
            ),
            format!(
                "thread '{thread_name}' panicked at '{panic_message}', {panic_location}\n{bt:?}"
            ),
            format!(
                "----------------------------------------\nchat-bubbles version {} crashed at \
                 {}\nthread '{}' panicked at '{}', {}\n-- backtrace --\n{:#?}",
    ...
    unsafe {
        let title = CString::new("chat-bubbles crashed!").unwrap();
